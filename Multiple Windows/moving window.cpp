...
namespace MovingWindow
{
	...
	LRESULT CALLBACK messageHandler(HWND window,UINT message,WPARAM argW,LPARAM argL)
	{
		...
		static RECT oldClient,newClient,intersection;	// in screen physical coordinates
		...
		case WM_CREATE:
			...
			return 0;
		case WM_MOVE:
			GetClientRect(window,&newClient);
			if(oldClient.right != 0)
				windowCounter++,windowCounter--;
			ClientToScreen(window,(POINT*)&newClient.left);
			ClientToScreen(window,(POINT*)&newClient.right);
			r.left = (short)LOWORD(argL);
			r.top = (short)HIWORD(argL);
			IntersectRect(&intersection,&oldClient,&newClient);
			ScreenToClient(window,(POINT*)&intersection.left);
			ScreenToClient(window,(POINT*)&intersection.right);
			InvalidateRect(window,&newClient,TRUE);
			ValidateRect(window,&intersection);
			UpdateWindow(window);
			CopyRect(&oldClient,&newClient);
			return 0;
		//case WM_WINDOWPOSCHANGED:
		//	//((WINDOWPOS*)argL)->flags |= SWP_NOCOPYBITS;
		//	InvalidateRect(window,NULL,FALSE);
		//	newClient.left = ((WINDOWPOS*)argL)->x;
		//	newClient.top = ((WINDOWPOS*)argL)->y;
		//	newClient.right = ((WINDOWPOS*)argL)->x + ((WINDOWPOS*)argL)->cx;
		//	newClient.bottom = ((WINDOWPOS*)argL)->y + ((WINDOWPOS*)argL)->cy;
		//	IntersectRect(&intersection,&oldClient,&newClient);
		//	ScreenToClient(window,(POINT*)&intersection.left);
		//	ScreenToClient(window,(POINT*)&intersection.right);
		//	ValidateRect(window,&intersection);
		...
		case WM_ERASEBKGND:
			{
				...
				SelectObject(dc,CreatePen(PS_SOLID,1,RGB(255,255,0)));
				//SetROP2(dc,R2_XORPEN);
					...
					PolyBezier(dc,controlPoints,length(controlPoints));
					}
				ReleaseDC(window,dc);
